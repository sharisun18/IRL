...
//    }
     ...
     */
    public LinearStateDifferentiableRF runIRL(String pathToEpisodes) {

        //create reward function features to use
        LocationFeatures features = new LocationFeatures(this.domain, Nloc);
        ...
        // ---------------- Got parameters for defining the IRL problem ---------------- //
    ...
    public void visualizeRF(LinearStateDifferentiableRF rf) {

        // planner
        DifferentiableSparseSampling dplanner = new DifferentiableSparseSampling(this.domain, rf, 0.99, new SimpleHashableStateFactory(), 10, -1, 10);
        ...
        ValueFunctionVisualizerGUI gui = GridWorldDomain.getGridWorldValueFunctionVisualization(
                ...
                maxX,
                maxY,
                new RewardValueProjection(rf),                                      // GET VALUE FUNCTION
                new GreedyQPolicy(dplanner)                 // GET POLICY
        );
    ...
    public GreedyQPolicy getPolicy(LinearStateDifferentiableRF rf) {

        DifferentiableSparseSampling dplanner = new DifferentiableSparseSampling(this.domain, rf, 0.99, new SimpleHashableStateFactory(), 10, -1, 10);

        return new GreedyQPolicy(dplanner);
    }
     ...
     */
    protected State basicState(){                                       // ------- ENVIRONMENT ------- //

        GridWorldState s = new GridWorldState(
                new GridAgent(0, 0),
                new GridLocation(0, 0, 1, "loc0"),
                new GridLocation(0, 5, 2, "loc1"),
                new GridLocation(5, 5, 3, "loc2"),
                new GridLocation(5, 0, 4, "loc3"),

                new GridLocation(1, 0, 0, "loc4"),
                new GridLocation(1, 2, 0, "loc5"),
                new GridLocation(1, 5, 0, "loc6"),
                new GridLocation(4, 1, 0, "loc7"),
                new GridLocation(4, 4, 0, "loc8")
        );
...
//        ex.launchExplorer(); //choose this to record demonstrations
        ex.launchSavedEpisodeSequenceVis("demos"); //choose this review the demonstrations that you've recorded
//        ex.runIRL("demos"); //choose this to run MLIRL on the demonstrations and visualize the learned reward function and policy
